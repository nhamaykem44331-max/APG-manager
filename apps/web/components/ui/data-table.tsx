...
export function DataTable<T>({
  ...
  const headerPadding = compact ? 'bg-muted/50 dark:bg-muted/20 px-2.5 py-2 text-[11.5px]' : 'bg-muted/50 dark:bg-muted/20 px-3.5 py-2.5 text-[12px]';
    ...
    <div className="flex flex-col w-full">
      ...
      >
            ...
            <tbody>
              {isLoading ? (
                ...
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={`skeleton-${i}`} className="border-b border-border last:border-0 bg-card dark:bg-background">
                    {columns.map((col, j) => (
                ...
                // Empty state
                <tr>
                  <td colSpan={columns.length} className="bg-card dark:bg-background py-10 text-center text-[12px] text-muted-foreground">
                    {emptyMessage}
                  </td>
                </tr>
              ) : (
                ...
                data.map((row, i) => (
                  <tr
                    ...
                    className={cn(
                      'border-b border-border last:border-0 bg-card dark:bg-background transition-colors duration-100 group',
                      onRowClick ? 'cursor-pointer hover:bg-accent/60' : 'hover:bg-accent/30'
                    )}
                  >
                    {columns.map((col, j) => (
        ...
        {(pageCount !== undefined && pageCount > 1) && (
          <div className="flex items-center justify-between border-t border-border bg-card dark:bg-background px-3.5 py-2.5">
