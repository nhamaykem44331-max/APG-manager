...
const WINDOWS_1252_UNICODE_TO_BYTE = new Map<number, number>([
...
const MOJIBAKE_MARKERS = ['Ã', 'Â', 'Ä', 'â', 'Æ°', 'á»', 'áº', 'Ãƒ'];

function encodeWindows1252Bytes(value: string) {
  return Uint8Array.from(Array.from(value, (char) => {
    const code = char.codePointAt(0) ?? 0x3f;
    if (code <= 0xff) return code;
    return WINDOWS_1252_UNICODE_TO_BYTE.get(code) ?? 0x3f;
  }));
}

function decodeLegacyUtf8Mojibake(value: string) {
  return new TextDecoder('utf-8', { fatal: false }).decode(encodeWindows1252Bytes(value));
}

export function normalizeLegacyVietnameseText(value?: string | null): string {
  if (!value) return '';
  ...
  let current = value;
  for (let i = 0; i < 2; i += 1) {
    if (!MOJIBAKE_MARKERS.some((marker) => current.includes(marker))) {
      break;
    }

    const decoded = decodeLegacyUtf8Mojibake(current);
